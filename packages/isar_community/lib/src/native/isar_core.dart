...
void _initializePath(String? libraryPath) {
  ...
  if (Platform.isIOS) {
    ...
    try {
      ...
      // When using Swift Package Manager, the dylib is linked via the
      // xcframework plugin. The @_silgen_name reference in
      // IsarFlutterLibsPlugin keeps it loaded into the process.
      dylib = DynamicLibrary.process();
    }
  } else {
    dylib = DynamicLibrary.open(libraryPath!);
